...
const QUESTION_POOL = [
  ...
  {
    question: "Welche Sprache mussten die Kinder in Magdeburg sprechen?",
    options: ["Deutsch", "Latein", "Griechisch", "Hebräisch"],
    answer: 1,
    explanation: "In dieser Schule mussten alle Kinder Latein sprechen. Das war damals an solchen Schulen üblich."
  },
  {
...
export default function MartinLutherQuiz() {
  ...
  const progress = useMemo(() => ((currentIndex + (isFinished ? 1 : 0)) / QUESTIONS_PER_ROUND) * 100, [currentIndex, isFinished]);
        ...
        <div className="mx-auto max-w-3xl">
          <Card className="rounded-3xl border-0 shadow-xl">
            <CardHeader className="pb-4 text-center">
              ...
              <CardTitle className="text-3xl font-bold">Runde beendet</CardTitle>
              <p className="text-slate-600">Du hast 5 zufällige Fragen aus einem Pool von {QUESTION_POOL.length} Fragen beantwortet.</p>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="rounded-2xl bg-slate-100 p-6 text-center">
                ...
                <div className="mt-2 text-5xl font-bold">{score} / {QUESTIONS_PER_ROUND}</div>
                <div className="mt-2 text-lg text-slate-700">{percentage}% richtig</div>
              </div>

              <Button className="h-12 w-full rounded-2xl text-base" onClick={startNewRound}>
          ...
          <CardHeader className="space-y-4 pb-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <CardTitle className="text-3xl font-bold">Frage {currentIndex + 1} von {QUESTIONS_PER_ROUND}</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-3">
              {currentQuestion.options.map((option, index) => {
                ...
                let className = "h-auto min-h-14 justify-start whitespace-normal rounded-2xl border px-4 py-4 text-left text-base";
            ...
            {hasAnswered && (
              <div className={`rounded-2xl p-4 ${selectedIndex === currentQuestion.answer ? "bg-green-50" : "bg-red-50"}`}>
                ...
                <Button className="mt-4 rounded-2xl" onClick={handleNextQuestion}>
